/*
  # Add Approval Workflow and Installments Support

  ## Overview
  This migration adds support for committee approval workflows, funding installments,
  and enhanced rejection tracking.

  ## 1. New Tables

  ### funding_installments
  - Tracks approved funding broken into installments
  - Supports up to 3 regular installments + optional BGRC installment
  - Fields: id, case_id, label, amount, due_date, status, created_at, updated_at

  ## 2. Table Modifications

  ### committee_reviews
  - Add outcome field (Pending, Approved, Rejected, Need_More_Info, Deferred)
  - Add approved_amount field
  - Add decision_date field
  - Add comments field

  ### rejection_reasons
  - Add reason_category field (Medical, Financial, Other)
  - Add rejection_level field (NFI, BRC, BRRC, BGRC, BCRC, Other)
  - Add communication_status field
  - Add referring_hospital field
  - Add case_summary field

  ### financial_case_details
  - Add approved_amount field to store final approved funding

  ## 3. Security
  - RLS policies for funding_installments
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'committee_reviews' AND column_name = 'outcome'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'committee_reviews' AND column_name = 'approved_amount'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'committee_reviews' AND column_name = 'decision_date'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'committee_reviews' AND column_name = 'comments'
  ) THEN
    ALTER TABLE committee_reviews ADD COLUMN comments text;
  END IF;
END $$;

-- Update the decision field constraint to match new outcomes
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'rejection_reasons' AND column_name = 'rejection_level'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'rejection_reasons' AND column_name = 'communication_status'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'rejection_reasons' AND column_name = 'referring_hospital'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'rejection_reasons' AND column_name = 'case_summary'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'financial_case_details' AND column_name = 'approved_amount'
  ) THEN
