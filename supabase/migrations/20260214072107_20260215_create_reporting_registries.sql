/*
  # Create Reporting Admin Registries

  1. New Tables
    - `kpi_catalog`
      - `id` (uuid, primary key)
      - `code` (text, unique - KPI identifier)
      - `name` (text - display name)
      - `description` (text)
      - `data_type` (text - 'Numeric', 'Percentage', 'Text')
      - `calculation_method` (text - how KPI is calculated)
    ...
    - `dataset_registry`
      - `id` (uuid, primary key)
      - `code` (text, unique - dataset identifier)
      - `name` (text - display name)
      - `description` (text)
      - `source_table` (text - database table/view)
    ...
    - `template_registry`
      ...
      - `name` (text - display name)
      - `description` (text)
      - `template_type` (text - 'Dashboard', 'Export', 'Alert')
      - `version` (text)
    ...
    - `template_bindings`
      - `id` (uuid, primary key)
      - `template_id` (uuid, foreign key to template_registry)
      - `kpi_id` (uuid, foreign key to kpi_catalog)
      - `dataset_id` (uuid, foreign key to dataset_registry)
      - `field_name` (text - field in template)
      - `mapping_config` (jsonb - transformation/mapping rules)
      - `sort_order` (integer)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Indexes
    - Indexes on codes and foreign keys for query performance
    - Index on is_active for filtering

  3. Security
    - Enable RLS on all tables
    - Admin only can view/modify registries
*/
...
CREATE POLICY "Admin can view KPI catalog"
  ON kpi_catalog FOR SELECT
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can manage KPI catalog"
  ON kpi_catalog FOR INSERT
  TO authenticated
  WITH CHECK (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can update KPI catalog"
  ON kpi_catalog FOR UPDATE
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin')
  WITH CHECK (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can delete KPI catalog"
  ON kpi_catalog FOR DELETE
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can view dataset registry"
  ON dataset_registry FOR SELECT
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can manage dataset registry"
  ON dataset_registry FOR INSERT
  TO authenticated
  WITH CHECK (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can update dataset registry"
  ON dataset_registry FOR UPDATE
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin')
  WITH CHECK (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can delete dataset registry"
  ON dataset_registry FOR DELETE
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can view template registry"
  ON template_registry FOR SELECT
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can manage template registry"
  ON template_registry FOR INSERT
  TO authenticated
  WITH CHECK (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can update template registry"
  ON template_registry FOR UPDATE
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin')
  WITH CHECK (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can delete template registry"
  ON template_registry FOR DELETE
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can view template bindings"
  ON template_bindings FOR SELECT
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can manage template bindings"
  ON template_bindings FOR INSERT
  TO authenticated
  WITH CHECK (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can update template bindings"
  ON template_bindings FOR UPDATE
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin')
  WITH CHECK (auth.jwt()->>'role' = 'admin');

CREATE POLICY "Admin can delete template bindings"
  ON template_bindings FOR DELETE
  TO authenticated
  USING (auth.jwt()->>'role' = 'admin');
