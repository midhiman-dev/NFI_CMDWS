/*
  # Create Finance Input Tables

  1. New Tables
    - `donations_ledger_entries`: Track donations received
      - `id` (uuid, primary key)
      - `entry_date` (date, required)
      - `donor_name` (text, required)
      - `amount` (numeric, required, > 0)
      - `category` (text: 'Cash', 'Check', 'Online')
      - `notes` (text, optional)
      - `created_at`, `updated_at`

    - `bank_fd_balance_snapshots`: Bank and FD balance snapshots
      - `id` (uuid, primary key)
      - `snapshot_date` (date, required)
      - `account_name` (text, required)
      - `account_number` (text)
      - `balance` (numeric, required, >= 0)
      - `account_type` (text: 'Savings', 'Current', 'FD')
      - `notes` (text, optional)
      - `created_at`, `updated_at`
    ...
    - `expense_transactions`: Expense transactions
      ...
      - `amount` (numeric, required, > 0)
      - `category` (text, required: 'Program', 'Operations')
  ...
  3. Indexes
    - Index on entry_date/snapshot_date/transaction_date for filtering
*/
...
CREATE INDEX idx_donations_entry_date ON donations_ledger_entries(entry_date);
CREATE INDEX idx_bank_snapshot_date ON bank_fd_balance_snapshots(snapshot_date);
CREATE INDEX idx_expense_transaction_date ON expense_transactions(transaction_date);
