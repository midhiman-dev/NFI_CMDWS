/*
  # Add Hospital Process Maps table

  1. New Tables
    - `hospital_process_maps`
      - `id` (uuid, primary key)
      - `hospital_id` (uuid, foreign key to hospitals)
      - `process_type` (text: BRC, BRRC, BGRC, BCRC, NON_BRC)
      - `is_active` (boolean, default true)
      - `effective_from_date` (date, for when the mapping becomes active)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Indexes
    - Unique index on (hospital_id) to ensure one mapping per hospital

  3. Security
    - Enable RLS on `hospital_process_maps` table
    - Add policy for admins to read all mappings
    - Add policy for admins to create mappings
    - Add policy for admins to update mappings
    - Add policy for admins to delete mappings
*/
