/*
  # NFI Case Management System - Complete Database Schema

  ## Overview
  This migration creates the complete database schema for the National Fetal Institute (NFI) 
  case management system for handling BGRC (Baal Gandhari Ratan Certificate) applications.

  ## 1. New Tables

  ### Core Entity Tables
  - `users` - System users (hospital staff, verifiers, committee members, admins)
    - id, email, full_name, role, hospital_id, is_active, created_at, updated_at
  
  - `hospitals` - Registered hospitals in the network
    - id, name, code, city, state, contact_person, phone, email, is_active, created_at
  
  - `bgrc_cycles` - BGRC cycles/rounds for case processing
    - id, cycle_name, start_date, end_date, is_active, created_at
  
  - `cases` - Main case records
    - id, case_number, hospital_id, bgrc_cycle_id, process_type, case_status, 
      created_by, created_at, updated_at, last_action_at, submitted_at, reviewed_at, decision_at

  ### Case Detail Tables
  - `beneficiary_profiles` - Patient demographic information
    - case_id, baby_name, gender, dob, birth_weight_kg, current_weight_kg, morbidity, mortality
  
  - `family_profiles` - Family information
    - case_id, father_name, mother_name, phone, address, city, state, pincode, income_band
  
  - `clinical_case_details` - Clinical information
    - case_id, diagnosis, summary, doctor_name, nicu_days
  
  - `financial_case_details` - Financial information
    - case_id, nicu_cost, pharmacy_cost, other_charges, total_billed, discount, final_bill_amount

  ### Document Management Tables
  - `document_templates` - Document requirement templates by process type
    - id, process_type, category, doc_type, mandatory_flag, condition_notes, display_order
  
  - `document_metadata` - Actual document records
    - id, case_id, category, doc_type, file_name, file_type, size, status, 
      uploaded_at, uploaded_by, verified_at, verified_by, notes, file_url

  ### Workflow Tables
  - `audit_events` - Complete audit trail
    - id, case_id, timestamp, user_id, user_role, action, notes
  
  - `followup_schedules` - Follow-up tracking
    - id, case_id, follow_up_type, scheduled_date, completed_date, outcome, notes
  
  - `committee_reviews` - Committee decisions
    - id, case_id, review_date, decision, amount_sanctioned, remarks, reviewed_by
  
  - `rejection_reasons` - Rejection tracking
    - id, case_id, rejection_date, reason_category, detailed_reason, rejected_by

  ## 2. Security
  - Enable Row Level Security (RLS) on all tables
  - Policies based on user role and ownership
  - Hospital staff can only see their hospital's cases
  - Verifiers and admins have broader access
  - Audit events are read-only for non-admins
...
CREATE POLICY "Hospital staff can view their hospital's cases"
  ON cases FOR SELECT
  TO authenticated
  USING (
    hospital_id IN (
...
CREATE POLICY "Hospital staff can update their hospital's cases"
  ON cases FOR UPDATE
  TO authenticated
  USING (
    hospital_id IN (
