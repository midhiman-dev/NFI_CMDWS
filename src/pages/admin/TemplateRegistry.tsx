...
export function TemplateRegistryPage() {
  ...
  const [items, setItems] = useState<TemplateRegistry[]>([]);
  ...
  const loadItems = async () => {
    try {
      setLoading(true);
      const data = await provider.listTemplateRegistry();
      setItems(data);
    } catch (error) {
      console.error('Error loading templates:', error);
      showToast('Failed to load template registry', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await provider.updateTemplate(editingId, formData);
        showToast('Template updated', 'success');
      } else {
        await provider.createTemplate(formData);
  ...
  const handleEdit = (item: TemplateRegistry) => {
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-[var(--nfi-text)]">Template Registry</h1>
          <p className="text-[var(--nfi-text-secondary)] mt-1">Create and manage report templates</p>
        </div>
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              <div className="text-center py-8">
                ...
                <p className="text-[var(--nfi-text-secondary)]">Loading templates...</p>
              </div>
            ) : items.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-[var(--nfi-text-secondary)]">No templates yet</p>
              </div>
            ) : (
                  ...
                  <tbody>
                    {items.map((item) => (
                      <tr key={item.id} className="border-b border-[var(--nfi-border)] hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 text-[var(--nfi-text)]">{item.version}</td>
